...
def merge_two_stacks(stack_1, stack_2, x_percentage):
    ...
    for img1, img2 in zip(stack_1, stack_2):
        ...
        merged_img = numpy.hstack((img1[:, 0:col_number], img2[:, col_number:]))
...
cv2.createTrackbar('horizontal amount', 'blended', 50, 100, trackbar_changed_do_nothing)
...
while True:
    ...
    result_stack = merge_two_stacks(apple_stacks, orange_stacks, cv2.getTrackbarPos('horizontal amount', 'blended') / 100)
