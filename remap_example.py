...
def _water_map_function(r, c, c_r, c_c, squared_radius, amplitude, wavelenth, phase):
    ...
    if squared_distance > squared_radius:
        ...
        amount = amplitude * \
            numpy.sin(distance / wavelenth * numpy.pi * 2 - phase)
...
def water_ripple_map(rows, cols, amplitude, wavelenth, phase):
    ...
    water_map = numpy.frompyfunc(lambda r, c: _water_map_function(
        r, c, c_r, c_c, squared_radius, amplitude, wavelenth, phase), 2, 2).outer(numpy.arange(rows), numpy.arange(cols))
...
def twist_ripple_map(rows, cols, angle):
    ...
    water_map = numpy.frompyfunc(lambda r, c: _twist_map_function(
        r, c, c_r, c_c, squared_radius, angle), 2, 2).outer(numpy.arange(rows), numpy.arange(cols))
    return water_map[0].astype(numpy.float32), water_map[1].astype(numpy.float32)
