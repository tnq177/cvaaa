...
cv2.createTrackbar('scale', 'affine', 100, 500, trackbar_changed_do_nothing)
...
while True:
    ...
    # Now try to show the whole image in case it's transformed out of image bound
    ...
    new_transformed_img = cv2.warpAffine(img, M, (int(max_col), int(max_row)))
